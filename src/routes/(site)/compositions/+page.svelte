...
<svelte:head>
  ...
  <meta
    name="description"
    content="Explore the Swetaranyam Sisters' comprehensive research on Carnatic compositions, including chronological mapping, comparative interpretation, and theme-based curation."
  />
</svelte:head>
...
<div class="py-14">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="mb-16">
      <h2 class="font-display text-3xl text-gold-200 mb-8 text-center">
      ...
      <div class="grid gap-8 lg:grid-cols-2">
        {#each aspects as aspect}
          ...
          <article
            class="rounded-2xl border border-maroon-700 bg-maroon-900/60 p-6 card-hover"
          >
            <div class="flex items-start gap-4 mb-4">
              <div
                class="w-12 h-12 rounded-full bg-gold-600 flex items-center justify-center shrink-0"
              >
                <IconComponent class="w-6 h-6 text-maroon-900" />
              </div>
              <div class="flex-1">
                <h3 class="font-display text-xl text-gold-200 mb-2">
                  {aspect.title}
                </h3>
              </div>
            </div>
            <div class="text-amber-100/90 leading-relaxed">
              ...
              {#if aspect.examples}
                <ul
                  class="list-disc list-inside space-y-1 text-sm text-amber-100/85 mb-3"
                >
    ...
    <div
      class="rounded-2xl border border-maroon-700 bg-maroon-800/60 p-8 mb-16"
    >
      <div class="flex items-start gap-6">
        <div
          class="w-16 h-16 rounded-full bg-gold-600 flex items-center justify-center shrink-0"
        >
          <Award class="w-8 h-8 text-maroon-900" />
        </div>
        <div class="flex-1">
          <h3 class="font-display text-2xl text-gold-200 mb-4">
            {educationalValue.title}
          </h3>
          ...
          {#if educationalValue.exampleStudy}
            <div class="bg-maroon-900/40 rounded-lg p-6 mb-6">
              ...
              <p class="text-amber-100/90">
                <strong class="text-gold-200"
                  >{educationalValue.exampleStudy.title}</strong
                >
                → {educationalValue.exampleStudy.description}
              </p>
          ...
          <div class="grid md:grid-cols-3 gap-6">
            {#each educationalValue.audiences as audience}
              ...
              <div class="text-center">
                <div
                  class="w-12 h-12 rounded-full bg-maroon-700 flex items-center justify-center mx-auto mb-3"
                >
                  <AudienceIcon class="w-6 h-6 text-gold-400" />
                </div>
                <h4 class="font-semibold text-gold-300 mb-2">
                  {audience.title}
                </h4>
    ...
    {#if methodology}
      <div
        class="rounded-2xl border border-maroon-700 bg-maroon-900/60 p-8 mb-16"
      >
        <h3 class="font-display text-2xl text-gold-200 mb-6 text-center">
          {methodology.title}
        </h3>
        <div class="grid md:grid-cols-2 gap-8">
          {#each methodology.sections as section}
            ...
            <div>
              <h4
                class="font-semibold text-gold-300 mb-4 flex items-center gap-2"
              >
    ...
    <div class="mt-16 text-center">
      <div
        class="rounded-2xl border border-maroon-700 bg-maroon-800/60 p-8 max-w-4xl mx-auto"
      >
        ...
        <div class="grid md:grid-cols-2 gap-6 text-left">
          {#each impact.opportunities as opportunity}
            <div>
              <h3 class="font-semibold text-gold-300 mb-2">
                {opportunity.title}
              </h3>
